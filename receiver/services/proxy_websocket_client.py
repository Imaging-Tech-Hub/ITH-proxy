...
class ProxyWebSocketClient:
    ...
    def _check_node_health_sync(self) -> list:
        ...
        try:
            ...
            from receiver.commands.dicom_send_commands import VerifyNodeConnectionCommand
            ...
            for node in nodes:
                if not node.is_active:
                    logger.info(f"⏭️  Skipping inactive node: {node.name} (is_active={node.is_active})")
                    continue
...
def get_websocket_client() -> Optional[ProxyWebSocketClient]:
    ...
    """
    from django.conf import settings
    from receiver.services.event_handlers import (
    ...
    client.register_event_handler('session.dispatch', SessionDispatchHandler().handle)
    client.register_event_handler('scan.dispatch', ScanDispatchHandler().handle)
    client.register_event_handler('subject.dispatch', SubjectDispatchHandler().handle)
    client.register_event_handler('scan.new_scan_available', NewScanAvailableHandler().handle)

    client.register_event_handler('proxy.config_changed', ProxyConfigChangedHandler().handle)
    client.register_event_handler('proxy.nodes_changed', ProxyNodesChangedHandler().handle)
    client.register_event_handler('proxy.status_changed', ProxyStatusChangedHandler().handle)

    client.register_event_handler('session.deleted', SessionDeletedHandler().handle)
    client.register_event_handler('scan.deleted', ScanDeletedHandler().handle)
    client.register_event_handler('subject.deleted', SubjectDeletedHandler().handle)
